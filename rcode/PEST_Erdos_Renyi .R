...
#PARAMETERS OF THE SIMULATED NETWORK --  DIMITRIS: how should we change this
n_nodes = c(50,100,150,200)

#MARTA PLAYS WITH THIS
#density is constant here
n_edges = list(c(1000, 500, 350), 
               c(2000, 1000, 700),
               c(3000, 1500, 1050),
               c(4000, 2000, 1400) )

#avg degree constant (more realistic scenario)
...
#play with the below function to check 1)the mean degree avg and how to recruit
...
ggsave(filename =  paste(my_dir_figs,"e_r_min_max_degree.pdf", sep = ""),
       plot = gg)



########
#WEIGHTS# (we decided to drop this for now)
#########
...
##########################################
#COMPUTE Barrat et al. (2004) coefficient# --> you can only use this if you add the weights
##########################################
###################################################################
...
ggsave(filename =  paste(my_dir_figs,"e_r_dens_nets.pdf", sep = ""),
       plot = gg)
...
#########################
#  PLOT OF THE NETWORK  # --> need to redo this
#########################

#example for one graph
df = reshape2::melt(snow_samp[[1]][[1]][[1]])
