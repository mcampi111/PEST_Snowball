...
prova<- my_snowball_fun(G_d_to_net[[1]][[3]],
                        n_seed = n_seed,
                        lsmi_union, 
                        n_waves = n_waves , 
                        delta = delta)
...
n_ratio = n_nodes[1]/n_nei[[1]]


p = 0.2 # DIMITRIS: how should we change this

#0 = c(0.1, 0.2, 0.5, 0.7)


G_d<- lapply(1:length(n_nodes), function(i)
         lapply(1:length(n_nei[[i]]),function(j)
            sample_smallworld(dim = 1,
                             size = n_nodes[i], 
                             nei = n_nei[[i]][[j]], 
                             p = p,
                             loops = FALSE,
...
min_degree = do.call(rbind,map( sapply(1:length(n_nodes), function(i)
  sapply(1:length(n_nei[[i]]),function(j) min(igraph::degree(G_d[[i]][[j]])) )), unlist ))

max_degree = do.call(rbind,map( sapply(1:length(n_nodes), function(i)
  sapply(1:length(n_nei[[i]]),function(j) max(igraph::degree(G_d[[i]][[j]])) )), unlist ))

mean_degree = do.call(rbind,map( sapply(1:length(n_nodes), function(i)
  sapply(1:length(n_nei[[i]]),function(j) mean(igraph::degree(G_d[[i]][[j]])) )), unlist ))
...
df_degree_r_names = do.call(rbind, map(sapply(1:length(n_nodes), function(i)
  sapply(1:length(n_nei[[i]]),function(j) 
    paste("n. nodes ", n_nodes[i], " n. neigh. ", n_nei[[i]][[j]], sep = "") )), unlist))
...
ggsave(filename =  paste(my_dir_figs,"s_w_min_max_degree.pdf", sep = ""),
       plot = gg)
...
g_dens<- lapply(1:length(n_nodes), function(i)
  sapply(1:length(n_nei[[i]]),function(j)  edge_density(G_d[[i]][[j]], loops=TRUE) ))
...
ggsave(filename =  paste(my_dir_figs,"s_w_dens_nets.pdf", sep = ""),
       plot = gg)
...
G_d_to_net <-  lapply(1:length(n_nodes), function(i)
  lapply(1:length(n_nei[[i]]),function(j) igraph_to_network(G_d[[i]][[j]]) ))
...
#FOR MARTA TO CHECK
# snow_samp[[1]][[1]][[1]][[1]] # first - n_nodes, second: n_nei, third seed and fourth for the wave
# 
# #1 seed
...
n_seed_wave_unique = lapply(1:length(n_nodes), function(i)
                       lapply(1:length(n_nei[[i]]),function(j)  do.call(rbind, n_seed_per_wave_unique[[i]][[j]]) ))

n_seed_wave_recr = lapply(1:length(n_nodes), function(i)
                     lapply(1:length(n_nei[[i]]),function(j)   do.call(rbind, n_seed_per_wave_recr[[i]][[j]])[,-1] ))
...
r_name_df_seed = lapply(1:length(n_nei), function(h) lapply(1:length(n_nei[[h]]), function(j) 
          sapply(1:5, function(i) paste("seed ", i, ", n. neigh. ", n_nei[[h]][[j]], sep = "" ))))
...
for (i in 1:length(n_nodes)){ n_seed_df_unique[[i]]$n_seed = rep(1:n_seed, length(n_nei[[1]]) ) }
for (i in 1:length(n_nodes)){ n_seed_df_unique[[i]]$n_nei = rep(n_nei[[i]], each = n_seed) }
...
for (i in 1:length(n_nodes)){ n_seed_df_recr[[i]]$n_seed = rep(1:n_seed, length(n_nei[[1]]) ) }
for (i in 1:length(n_nodes)){ n_seed_df_recr[[i]]$n_nei = rep(n_nei[[i]], each = n_seed) }
...
df_snow_unique = reshape2::melt(n_seed_df_new_unique, id = c("n_nodes","mean_degree" ,"n_seed", "n_nei"))
df_snow_unique = df_snow_unique %>%  mutate(value2 = value/n_nodes) 


df_snow_recr = reshape2::melt(n_seed_df_new_recr, id = c("n_nodes","mean_degree" ,"n_seed", "n_nei"))
...
ggsave(filename =  paste(my_dir_figs,"s_w_snow_0.1_recr.pdf", sep = ""),
       plot = gg)
