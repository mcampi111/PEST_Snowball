...
#If running after loading the environment- run from here, after loading libraries
...
df_with_stats <- df_snow_recr_comb %>%
  group_by(n_nodes,mean_degree,n_seed, n_nei, variable,L1)
# Plot the results
ggplot(df_with_stats, aes(x = variable, y = value2, color = as.factor(n_seed), group = n_seed)) +
  ...
  facet_grid(n_nodes ~ n_nei) + # Facet by both n_nodes and n_edges
  labs(
